...
export function renderPointer (scope) {
    scope.ctx.strokeStyle = COLORS.CURSOR; // green
    ...
    scope.ctx.stroke();
    scope.ctx.fillStyle = COLORS.LINES;
...
export function renderCanvas (scope) {
    scope.ctx.fillStyle = COLORS.CANVAS;
    ...
    for (let i = 0; i < CANVAS_DIMENSIONS.WIDTH; i += scope.keys.currentSnap) {
        if (scope.keys.hasSnap) {
            ...
            if (i % 100 === 0) {
                scope.ctx.strokeStyle = COLORS.LINES_BIG;
            } else {
                scope.ctx.strokeStyle = COLORS.LINES_SMALL;
            }
            scope.ctx.lineWidth = 0.5;
            scope.ctx.closePath()
            scope.ctx.stroke();
